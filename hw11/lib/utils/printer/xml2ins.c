...
static T_type xmlinstype(XMLNode *node) {
...
static Temp_temp xmlinstemp(XMLNode *t) {
...
static Temp_tempList xmlinstemplist(XMLNode *l) {
    XMLNode *t;
    if (!l || l->children.size == 0) return NULL;
#ifdef __DEBUG
    fprintf(stderr, "---tmp---xmlinstemplist: %s, size=%d\n", l->tag, l->children.size);
#endif
...
static Temp_labelList xmlinslabellist(XMLNode *jumps) {
#ifdef __DEBUG
    fprintf(stderr, "---tmp---xmlinslabellist: %s, size=%d\n", jumps->tag, jumps->children.size);
#endif
    assert(jumps || !strcmp(jumps->tag, "labelist"));
    if (jumps->children.size == 0) return NULL;
    if (jumps->children.size == 1) return Temp_LabelList(Temp_namedlabel(jumps->children.data[0]->inner_text), NULL);
    if (jumps->children.size == 2) {
        XMLNode *l1 = xmlgetchildnode(jumps, "labellist");
        return Temp_LabelList(Temp_namedlabel(jumps->children.data[0]->inner_text), xmlinslabellist(l1));
    }
    fprintf(stderr, "Invalid LabelList Element.\n");
    exit -2;
}

static AS_instr xmlgetins(XMLNode *ins) {
    ...
    if (!strcmp(tag, "oper")) {
        ...
        if (dst) dsttemplist=xmlinstemplist(dst->children.data[0]);
        XMLNode *src = xmlgetchildnode(ins, "src");
        if (src) srctemplist=xmlinstemplist(src->children.data[0]);
        XMLNode *jumps = xmlgetchildnode(ins, "jumps");
        if (jumps) as_jumps=AS_Targets(xmlinslabellist(jumps->children.data[0]));
        else as_jumps=AS_Targets(NULL);
        return AS_Oper(String(assem->inner_text), dsttemplist, srctemplist, as_jumps);
        ...
        return AS_Label(String(assem->inner_text), Temp_namedlabel(temp_label->inner_text));
        ...
        XMLNode *src = xmlgetchildnode(ins, "src");
        if (src) srctemplist=xmlinstemplist(src->children.data[0]);
        return AS_Move(String(assem->inner_text), dsttemplist, srctemplist);
    } else {
...
AS_instrList insxml2func(XMLNode *l) {
    ...
    for (int i=0; i<l->children.size; i++) {
        ...
        aslist = AS_splice(aslist, AS_InstrList(ins, NULL));
    }
#ifdef __DEBUG
