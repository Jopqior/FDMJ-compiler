...
A_stm A_Putnum(A_pos pos, A_exp e) {
  A_stm s = checked_malloc(sizeof(*s));
  s->pos = pos;
  s->kind = A_putnum;
  s->u.e = e;
...
A_exp A_NumConst(A_pos pos, float n) {
  A_exp e = checked_malloc(sizeof(*e));
  e->pos = pos;
  e->kind = A_numConst;
  e->u.num = n;
...
A_exp A_NewFloatArrExp(A_pos pos, A_exp size) {
  A_exp e = checked_malloc(sizeof(*e));
  e->pos = pos;
  e->kind = A_newFloatArrExp;
  e->u.e = size;
  return e;
}
