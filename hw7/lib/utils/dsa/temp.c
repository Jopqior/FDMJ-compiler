...
static Temp_tempList Temp_TempListadd(Temp_tempList tl, Temp_temp t) {
  for (Temp_tempList l = tl; l != NULL; l = l->tail) {
    if (l->head == t) return tl; // nothing to add
  }
...
Temp_tempList Temp_TempListInter(Temp_tempList a, Temp_tempList b) {
  ...
  while (scan) {
    ...
    if (found) result = Temp_TempList(scan->head, result);
    scan = scan->tail;
  }
  return result;
}
...
// add one to another list (if not already there)
static Temp_labelList Temp_LabelListadd(Temp_labelList tl, Temp_label t) {
...
Temp_labelList Temp_LabelListInter(Temp_labelList a, Temp_labelList b) {
  ...
  while (scan) {
    ...
    if (found) result = Temp_LabelList(scan->head, result);
    scan = scan->tail;
  }
  return result;
}
...
void showit(Temp_temp t, string r) {
  string s;
  if (t->type == T_int) s= String("int");
  else if (t->type == T_float) s= String("float");
  else assert(0);
