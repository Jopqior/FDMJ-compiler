...
static void pr_stm(FILE *out, T_stm stm, int d) {
  ...
  case T_JUMP:
    ...
    switch (format) {
      ...
      case IRP_xml:
        fprintf(out, "<Jump>\n");
        fprintf(out, "<label>%s</label>", Temp_labelstring(stm->u.JUMP.jump));
        fprintf(out, "</Jump>\n");
        break;
    }
    break;
...
static void pr_temp(FILE *out, Temp_temp t, int d) {
  switch (format) {
    case IRP_parentheses:
      fprintf(out, "Temp_namedtemp(%s,%s)", Temp_look(Temp_name(), t), getT_type(t->type));
      break;
    case IRP_xml:
      fprintf(stdout, "<temp><name>%s</name><type>%s</type></temp>", Temp_look(Temp_name(), t), getT_type(t->type));
      break;
  }
  return ;
} 
...
static void pr_templist(FILE *out, Temp_tempList tl, int d) {
  indent(out, d);
  if (!tl) {
    if (format==IRP_parentheses) fprintf(out, "NULL");
    return ;
  }
  switch (format) {
    case IRP_parentheses:
...
static void pr_exp(FILE *out, T_exp exp, int d) {
  ...
  case T_ESEQ:
    ...
    if (format==IRP_parentheses) fprintf(out, ",\n");
    else fprintf(out, "</Stm>\n<Exp>\n");
    pr_exp(out, exp->u.ESEQ.exp, d + 1);
    if (format==IRP_parentheses) fprintf(out, "\n");
    else fprintf(out, "</Exp>\n");
    indent(out, d);
    if (format==IRP_parentheses) fprintf(out, ")");
    else fprintf(out, "</Eseq>\n"); 
    break;
  case T_NAME:
    indent(out, d);
    switch (format) {
  ...
  case T_CONST:
    ...
    switch (format) {
      case IRP_parentheses:
        if (exp->type == T_int) fprintf(out, "T_IntConst(%i)", exp->u.CONST.i);
        else fprintf(out, "T_FloatConst(%f)", exp->u.CONST.f);
        break;
      case IRP_xml:
        if (exp->type == T_int) fprintf(out, "<IntConst>%i</IntConst>", exp->u.CONST.i);
        else fprintf(out, "<FloatConst>%f</FloatConst>", exp->u.CONST.f);
        break;
    }
    break;
  case T_CALL: {
...
void printIRP_StmList (FILE *out, T_stmList stmList, int d) {
  T_stmList l = stmList;
  if (l) {
    indent(out, d);
    switch (format) {
      case IRP_parentheses: fprintf(out, "T_StmList(\n"); break;
      case IRP_xml: fprintf(out, "<StmList>\n"); break;
    }
    pr_stm(out, l->head, d);
    l = l->tail;
    if (l) {
      ...
      indent(out, d);
      switch (format) {
        case IRP_parentheses: fprintf(out, ")"); break;
        case IRP_xml: fprintf(out, "</StmList>\n"); break;
      }
    } else {
      indent(out, d);
      switch (format) {
        case IRP_parentheses: fprintf(out, ", NULL)"); break;
        case IRP_xml: fprintf(out, "</StmList>\n"); break;
      }
    }
  } else {
...
void printIRP_FuncDecl(FILE *out, T_funcDecl funcDecl) {
...
void printIRP_FuncDeclList(FILE *out, T_funcDeclList funcDeclList) {
  ...
  switch (format) {
    case IRP_parentheses: fprintf(out, "T_FuncDeclList(\n"); break;
    case IRP_xml: fprintf(out, "<FuncDeclList>\n"); break;
  }
