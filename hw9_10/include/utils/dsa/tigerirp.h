...
T_stm T_Cjump(T_relOp op, T_exp left, T_exp right, Temp_label t, Temp_label f);
T_stm T_Move(T_exp, T_exp);
T_stm T_Exp(T_exp);
T_stm T_Return(T_exp);

T_exp T_Binop(T_binOp, T_exp, T_exp); // ret: T_int or T_float (inferred)
T_exp T_Mem(T_exp, T_type); // ret: T_int (ptr auto cast) or T_float, arg: type stored in memroy (T_int or T_float)
