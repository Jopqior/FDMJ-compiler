...
static T_stm reorder(expRefList rlist) {
  if (!rlist) return T_Exp(T_IntConst(0)); /* nop */
...
static struct stmExp do_exp(T_exp exp) {
  ...
  case T_ExtCALL:
    ...
    return StmExp(seq(reorder(get_extcall_rlist(exp)),
              T_Move(T_Temp(t), exp)), T_Temp(t));
  case T_CAST:
    return StmExp(reorder(ExpRefList(&exp->u.CAST, NULL)), exp); //FDMJ 2024 has a T_CAST
  default:
  ...
  /* we want false label to follow CJUMP */
    ...
    if (false) {
      ...
      last->tail->head = T_Cjump(s->u.CJUMP.op, s->u.CJUMP.left,
                     s->u.CJUMP.right, s->u.CJUMP.t, false);
...
static T_stmList getNext() {
  if (!global_block.stmLists)
    ...
    return T_StmList(T_Label(global_block.label),
             T_StmList(T_Return(T_IntConst(-1)), NULL)); //this return int is a dummy return, doesn't matter what type it is
  else {
