...
T_exp T_Mem(T_exp exp, T_type type) {
  T_exp p = (T_exp) checked_malloc(sizeof * p);
  p->kind = T_MEM;
  p->type = type;
  p->u.MEM = exp;
...
T_exp T_Temp(Temp_temp temp) {
  T_exp p = (T_exp) checked_malloc(sizeof * p);
  p->kind = T_TEMP;
  p->type = temp->type; // the type of a TEMP Exp is the same as the type of the TEMP
  p->u.TEMP = temp;
  return p;
}
...
T_exp T_Call(string id, T_exp obj, T_expList args, T_type type) {
  T_exp p = (T_exp) checked_malloc(sizeof * p);
  p->kind = T_CALL;
  p->type = type;
  p->u.CALL.id = id;
...
T_exp T_ExtCall(string id, T_expList args, T_type type) {
  T_exp p = (T_exp) checked_malloc(sizeof * p);
  p->kind = T_ExtCALL;
  p->type = type; // the type of an ExtCall is the same as the type of the function. If the external returns void, use T_int
