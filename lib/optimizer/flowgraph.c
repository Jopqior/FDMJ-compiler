...
G_graph FG_AssemFlowGraph(AS_instrList il) {
  ...
  for (; il != NULL; il = il->tail) {
    ...
    if (instr != NULL) {
      ...
      case I_OPER:
#ifdef __DEBUG
        fprintf(stderr, "instru->kind=%d, %d, %s\n", instr->kind, I_OPER, instr->u.OPER.assem); fflush(stderr);
        if (instr->u.OPER.jumps != NULL && instr->u.OPER.jumps->labels != NULL) {
      ...
      case I_LABEL:
#ifdef __DEBUG
        fprintf(stderr, "instru->kind=%d, %d, %s\n", instr->kind, I_LABEL, instr->u.LABEL.assem); fflush(stderr);
#endif
      ...
      case I_MOVE:
#ifdef __DEBUG
        fprintf(stderr, "instru->kind=%d, %d, %s\n", instr->kind, I_MOVE, instr->u.MOVE.assem); fflush(stderr);
#endif
  ...
  for (; jumpListHead != NULL; jumpListHead = jumpListHead->tail) {
    ...
    for (; labels != NULL; labels = labels->tail) {
      ...
      if (label != NULL) {
        ...
        // establish edge between this node and its jump target (if target exists)
        if (dest != NULL) G_addEdge(curr, dest); 
        else {
#ifdef __DEBUG
          fprintf(stderr, "Warning: jump to a non-existing label: %s\n", Temp_labelstring(label));
          fflush(stderr);
#endif
        } 
      }
