...
static void pr_stm(FILE *out, T_stm stm, int d) {
  ...
  case T_LABEL:
    indent(out, d - 2);
    fprintf(out, "%s:", S_name(stm->u.LABEL));
    break;
  case T_JUMP:
    indent(out, d);
    fprintf(out, "Jump ");
  ...
  case T_CJUMP:
    indent(out, d);
    fprintf(out, "Cjump %s", rel_oper[stm->u.CJUMP.op]);
    fprintf(out, ", ");
    pr_tree_exp(out, stm->u.CJUMP.left, d + 1);
    fprintf(out, ", ");
    pr_tree_exp(out, stm->u.CJUMP.right, d + 1);
    fprintf(out, ", ");
    fprintf(out, "%s, ", S_name(stm->u.CJUMP.t));
    fprintf(out, "%s", S_name(stm->u.CJUMP.f));
    break;
  case T_MOVE:
    indent(out, d);
    fprintf(out, "Move ");
...
static void pr_tree_exp(FILE *out, T_exp exp, int d) {
  ...
  case T_BINOP:
    ...
    fprintf(out, ", ");
    pr_tree_exp(out, exp->u.BINOP.right, 0);
    fprintf(out, ")");
    break;
  case T_MEM:
    fprintf(out, "Mem(");
    pr_tree_exp(out, exp->u.MEM, 0);
    fprintf(out, ")");
    break;
  case T_TEMP:
  ...
  case T_NAME:
    ...
    break;
  case T_CONST:
    if (exp->type == T_int) fprintf(out, "%i", exp->u.CONST.i);
    else fprintf(out, "%f", exp->u.CONST.f);
    break;
  case T_CALL: {
  ...
  fprintf(out, ":");
  if (exp->type == T_int) fprintf(out, "int");
  else fprintf(out, "float");
}

void printStm_StmList(FILE *out, T_stmList stmList, int d) {
  T_stmList l = stmList;
  if (l) {
    indent(out, d);
    pr_stm(out, l->head, d);
